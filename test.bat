...
if "!RUNWITHRACE!"=="" (
  ...
  echo Running standard tests... with race detector enabled and thus with -d=checkptr=1
)
...
go.exe test !RUNWITHRACE! !VERBOSE_GO_FLAG! !MOD_FLAG! ./...
...
go.exe test !RUNWITHRACE! -c !MOD_FLAG! -tags portmasterFirewalled -o dev_dns_test.exe .\internal\dnsbollocks\
if %ERRORLEVEL% equ 0 (
