...
func TestGenerateCertIfNeeded(t *testing.T) {
	...
	t.Run("WebUI TLS Disabled - Generates Cert Only For DoH IP", func(t *testing.T) {
		...
		cfg := &Config{
			...
			ListenUI:    "192.168.1.50:8080", // Different IP
		...
		uiIP := net.ParseIP("192.168.1.50")
		for _, ip := range leaf.IPAddresses {
			if ip.Equal(uiIP) {
				t.Errorf("Security/Spec violation: Web UI IP 192.168.1.50 was found in certificate SAN, but WebUIUseTLS is false")
			}
		}
	})
	...
	t.Run("WebUI TLS Disabled - Skips Regen When UI IP Changes", func(t *testing.T) {
		...
		cfg := &Config{
			ListenDoH:   "127.0.0.1:443",
			ListenUI:    "10.0.0.5:8080", // Changed from 192.168.1.50
			WebUIUseTLS: false,
		}
		s.liveConfig.Store(cfg)
